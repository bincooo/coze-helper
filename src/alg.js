...
function genSignature() {
    ...
    const timer = new Date().getTime() / ( (slice[pos = 297] << 8) + slice[pos = 297 + 1] ) >> 0; // 去小数

    const Ox0 = _0x2d72f8(0, timer.toString());
    const Ox1 = _0x2d72f8(Ox0, "www.coze.com/");

    const Ox2 = ( ( ( ( (slice[362] << 8) + slice[362 + 1] ) << 8 ) + slice[362 + 2] ) << 8 ) + slice[362 + 3];
    const Ox3 = (Ox1 % Ox2) * Ox2;
    const Ox4 = timer ^ Ox3 >>> 0;

    const bin = Ox4.toString(2);

    const num = 35393903335504;// parseInt(bin, 2);
    let i, i1; // 35393903335504 >> 2
    ...
    i1 = i1 + parseInt(params[ (slice[912] << 8) + slice[913] ]);
    i1 = num / i1;
    let i2 = i1 >> slice[168]; // 515 = 8240 >> 4
    i1 = slice[85] << 24 >> 24;
    i = i2 >> i1;
    i1 = slice[4] << 24 >> 24;
    i1 = i1 & i;
    i = i1 + slice[31] << 24 >> 24; // 65 = 0 + 65
    str += fromCharCode(i)
...
function genBogus() {
    ...
    const bodyHex = new alg_kit(true).update('{}').hex();
    const msTokenHex = new alg_kit(true).update("msToken=PWbBDDWPMjOEKP-_fpTGVTdknwPFZUsfMwzd3bhV_3-A87oHblcjDeonot27gkvjVdipokKWBVrVB_qnK9e_JDUvlV_hhiQJHZCxl6YlvaPgRAugN8VIuR9z5waEguE2").hex();
